...
chrome.tabs.executeScript(
  ...
  function (selection) {
    document.getElementById("selected").innerHTML =
      "Selected Text: " + selection[0].italics(); // Update selected text with user highlighted text
    selected_text = selection[0]; // Set variable 'selected_text' to selected string

    const searchWiki = async (search) => {
      const response = await fetch(
        `https://en.wikipedia.org/w/api.php?format=json&action=query&prop=extracts&exchars=500&explaintext&redirects=1&titles=${search}`
      ); // Call API with selected word
    ...
    searchWiki(selected_text).then((data) => {
      ...
      if (JSON.stringify(Object.values(data.query.pages)[0].missing)) {
        document.getElementById("contents").innerHTML =
          "No results :( <br> Please try again.";
      } else {
        ...
        document.getElementById("contents").innerHTML =
          formattedJSONString + "<br> <br>"; // Update the "contents" div with the formatted JSON string
...
function loadWiki() {
  // Open the relevant wikipedia page in a new tab
  chrome.tabs.create(
    { url: "https://en.wikipedia.org/wiki/" + selected_text },
    function (tab) {}
  );
}
